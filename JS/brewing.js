...
function updateBrewHTML() {
    for(let i = 0; i < flaskData.length; i++) {
        if(data.flaskTested[i]) {
            DOMCacheGetOrSet(`brewButton${i}`).innerText = `Brew\n-${formatSci(getTotalCost(baseBrewCost[i],D(1.01),buyAmounts[data.buyAmounts[0]],i))} Energy`
            DOMCacheGetOrSet(`brewButton${i}`).classList = data.greenEnergy.gte(getTotalCost(baseBrewCost[i],D(1.01),buyAmounts[data.buyAmounts[0]],i)) ? 'unlocked' : 'locked'
...
function brewJuice(i) {
    if(data.greenEnergy.lt(getTotalCost(baseBrewCost[i],D(1.01),buyAmounts[data.buyAmounts[0]],i))) return
    data.greenEnergy = data.greenEnergy.sub(getTotalCost(baseBrewCost[i],D(1.01),buyAmounts[data.buyAmounts[0]],i))
